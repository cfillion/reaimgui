...
DEFINE_ENUM(ImGui, Col_Header,                "Header* colors are used for CollapsingHeader, TreeNode, Selectable, MenuItem");
...
DEFINE_ENUM(ImGui, StyleVar_WindowMinSize,       "Minimum window size. This is a global setting. If you want to constraint individual windows, use SetNextWindowSizeConstraints().");
...
DEFINE_ENUM(ImGui, StyleVar_PopupRounding,       "Radius of popup window corners rounding. (Note that tooltip windows use WindowRounding)");
...
DEFINE_ENUM(ImGui, StyleVar_IndentSpacing,       "Horizontal indentation when e.g. entering a tree node. Generally == (FontSize + FramePadding.x*2).");
...
DEFINE_ENUM(ImGui, TableBgTarget_RowBg0, "Set row background color 0 (generally used for background, automatically set when ImGuiTableFlags_RowBg is used)");
...
// ImGuiColorEditFlags
DEFINE_ENUM(ImGui, ColorEditFlags_None,             "Flags for ColorEdit3() / ColorEdit4() / ColorPicker3() / ColorPicker4() / ColorButton()");
...
DEFINE_ENUM(ImGui, ColorEditFlags__OptionsDefault,  "Defaults Options. You can set application defaults using SetColorEditOptions(). The intent is that you probably don't want to override them in most of your calls. Let the user choose via the option menu and/or call SetColorEditOptions() once during startup.");
...
// ImGuiDragDropFlags
DEFINE_ENUM(ImGui, DragDropFlags_None,                     "Flags for ImGui::BeginDragDropSource(), ImGui::AcceptDragDropPayload()");
// BeginDragDropSource() flags
DEFINE_ENUM(ImGui, DragDropFlags_SourceNoPreviewTooltip,   "By default, a successful call to BeginDragDropSource opens a tooltip so you can display a preview or description of the source contents. This flag disable this behavior.");
DEFINE_ENUM(ImGui, DragDropFlags_SourceNoDisableHover,     "By default, when dragging we clear data so that IsItemHovered() will return false, to avoid subsequent user code submitting tooltips. This flag disable this behavior so you can still call IsItemHovered() on the source item.");
...
DEFINE_ENUM(ImGui, DragDropFlags_AcceptBeforeDelivery,     "AcceptDragDropPayload() will returns true even before the mouse button is released. You can then call IsDelivery() to test if the payload needs to be delivered.");
...
DEFINE_ENUM(ImGui, DragDropFlags_AcceptPeekOnly,           "For peeking ahead and inspecting the payload before delivery. Equivalent to ImGuiDragDropFlags_AcceptBeforeDelivery | ImGuiDragDropFlags_AcceptNoDrawDefaultRect.");

// ImGuiFocusedFlags
DEFINE_ENUM(ImGui, FocusedFlags_None,                "Flags for ImGui::IsWindowFocused()");
DEFINE_ENUM(ImGui, FocusedFlags_ChildWindows,        "IsWindowFocused(): Return true if any children of the window is focused");
DEFINE_ENUM(ImGui, FocusedFlags_RootWindow,          "IsWindowFocused(): Test from root window (top most parent of the current hierarchy)");
DEFINE_ENUM(ImGui, FocusedFlags_AnyWindow,           "IsWindowFocused(): Return true if any window is focused. Important: If you are trying to tell how to dispatch your low-level inputs, do NOT use this. Use 'io.WantCaptureMouse' instead! Please read the FAQ!");
...
// ImGuiInputTextFlags
DEFINE_ENUM(ImGui, InputTextFlags_None,                "Most of the ImGuiInputTextFlags flags are only useful for InputText() and not for InputFloatX, InputIntX, InputDouble etc.");
...
// ImGuiPopupFlags
DEFINE_ENUM(ImGui, PopupFlags_None,                    "Flags: for OpenPopup*(), BeginPopupContext*(), IsPopupOpen()");
DEFINE_ENUM(ImGui, PopupFlags_MouseButtonLeft,         "For BeginPopupContext*(): open on Left Mouse release. Guaranteed to always be == 0 (same as ImGuiMouseButton_Left)");
DEFINE_ENUM(ImGui, PopupFlags_MouseButtonRight,        "For BeginPopupContext*(): open on Right Mouse release. Guaranteed to always be == 1 (same as ImGuiMouseButton_Right)");
DEFINE_ENUM(ImGui, PopupFlags_MouseButtonMiddle,       "For BeginPopupContext*(): open on Middle Mouse release. Guaranteed to always be == 2 (same as ImGuiMouseButton_Middle)");
...
DEFINE_ENUM(ImGui, PopupFlags_AnyPopupId,              "For IsPopupOpen(): ignore the ImGuiID parameter and test for any popup.");
...
DEFINE_ENUM(ImGui, PopupFlags_AnyPopup,                "ImGuiPopupFlags_AnyPopupId | ImGuiPopupFlags_AnyPopupLevel");

// ImGuiSelectableFlags
DEFINE_ENUM(ImGui, SelectableFlags_None,             "Flags: for Selectable()");
...
// ImGuiSliderFlags
DEFINE_ENUM(ImGui, SliderFlags_None,            "for DragFloat(), DragInt(), SliderFloat(), SliderInt() etc.");
...
DEFINE_ENUM(ImGui, SliderFlags_Logarithmic,     "Make the widget logarithmic (linear otherwise). Consider using ImGuiSliderFlags_NoRoundToFormat with this if using a format-string with small amount of digits.");
...
// ImGuiTabBarFlags
DEFINE_ENUM(ImGui, TabBarFlags_None,                         "Flags: for BeginTabBar()");
...
DEFINE_ENUM(ImGui, TabBarFlags_NoCloseWithMiddleMouseButton, "Disable behavior of closing tabs (that are submitted with p_open != NULL) with middle mouse button. You can still repro this behavior on user's side with if (IsItemHovered() && IsMouseClicked(2)) *p_open = false.");
DEFINE_ENUM(ImGui, TabBarFlags_NoTabListScrollingButtons,    "Disable scrolling buttons (apply when fitting policy is ImGuiTabBarFlags_FittingPolicyScroll)");
...
// ImGuiTabItemFlags
DEFINE_ENUM(ImGui, TabItemFlags_None,                         "Flags: for BeginTabItem()");
...
DEFINE_ENUM(ImGui, TabItemFlags_SetSelected,                  "Trigger flag to programmatically make the tab selected when calling BeginTabItem()");
DEFINE_ENUM(ImGui, TabItemFlags_NoCloseWithMiddleMouseButton, "Disable behavior of closing tabs (that are submitted with p_open != NULL) with middle mouse button. You can still repro this behavior on user's side with if (IsItemHovered() && IsMouseClicked(2)) *p_open = false.");
DEFINE_ENUM(ImGui, TabItemFlags_NoPushId,                     "Don't call PushID(tab->ID)/PopID() on BeginTabItem()/EndTabItem()");
...
// ImGuiTableFlags
DEFINE_ENUM(ImGui, TableFlags_None, R"(for ImGui::BeginTable()
...
DEFINE_ENUM(ImGui, TableFlags_SizingFixedSame,            "Columns default to _WidthFixed or _WidthAuto (if resizable or not resizable), matching the maximum contents width of all columns. Implicitly enable ImGuiTableFlags_NoKeepColumnsVisible.");
...
DEFINE_ENUM(ImGui, TableFlags_SizingStretchSame,          "Columns default to _WidthStretch with default weights all equal, unless overriden by TableSetupColumn().");
...
// Clipping
DEFINE_ENUM(ImGui, TableFlags_NoClip,                     "Disable clipping rectangle for every individual columns (reduce draw command count, items will be able to overflow into other columns). Generally incompatible with TableSetupScrollFreeze().");
// Padding
DEFINE_ENUM(ImGui, TableFlags_PadOuterX,                  "Default if BordersOuterV is on. Enable outer-most padding. Generally desirable if you have headers.");
DEFINE_ENUM(ImGui, TableFlags_NoPadOuterX,                "Default if BordersOuterV is off. Disable outer-most padding.");
DEFINE_ENUM(ImGui, TableFlags_NoPadInnerX,                "Disable inner padding between columns (double inner padding if BordersOuterV is on, single inner padding if BordersOuterV is off).");
// Scrolling
DEFINE_ENUM(ImGui, TableFlags_ScrollX,                    "Enable horizontal scrolling. Require 'outer_size' parameter of BeginTable() to specify the container size. Changes default sizing policy. Because this create a child window, ScrollY is currently generally recommended when using ScrollX.");
DEFINE_ENUM(ImGui, TableFlags_ScrollY,                    "Enable vertical scrolling. Require 'outer_size' parameter of BeginTable() to specify the container size.");
// Sorting
DEFINE_ENUM(ImGui, TableFlags_SortMulti,                  "Hold shift when clicking headers to sort on multiple column. TableGetSortSpecs() may return specs where (SpecsCount > 1).");
DEFINE_ENUM(ImGui, TableFlags_SortTristate,               "Allow no sorting, disable default sorting. TableGetSortSpecs() may return specs where (SpecsCount == 0).");

// ImGuiTableColumnFlags
DEFINE_ENUM(ImGui, TableColumnFlags_None,                 "Flags: For TableSetupColumn()");
...
// ImGuiTableRowFlags
DEFINE_ENUM(ImGui, TableRowFlags_None,    "Flags: For TableNextRow()");
...
// ImGuiTreeNodeFlags
DEFINE_ENUM(ImGui, TreeNodeFlags_None,                 "Flags for TreeNode(), TreeNodeEx(), CollapsingHeader()");
DEFINE_ENUM(ImGui, TreeNodeFlags_Selected,             "Draw as selected");
DEFINE_ENUM(ImGui, TreeNodeFlags_Framed,               "Draw frame with background (e.g. for CollapsingHeader)");
DEFINE_ENUM(ImGui, TreeNodeFlags_AllowItemOverlap,     "Hit testing to allow subsequent widgets to overlap this one");
DEFINE_ENUM(ImGui, TreeNodeFlags_NoTreePushOnOpen,     "Don't do a TreePush() when open (e.g. for CollapsingHeader) = no extra indent nor pushing on ID stack");
...
DEFINE_ENUM(ImGui, TreeNodeFlags_OpenOnArrow,          "Only open when clicking on the arrow part. If ImGuiTreeNodeFlags_OpenOnDoubleClick is also set, single-click arrow or double-click all box to open.");
...
DEFINE_ENUM(ImGui, TreeNodeFlags_FramePadding,         "Use FramePadding (even for an unframed text node) to vertically align text baseline to regular widget height. Equivalent to calling AlignTextToFramePadding().");
...
DEFINE_ENUM(ImGui, WindowFlags_AlwaysUseWindowPadding,    "Ensure child windows without border uses style.WindowPadding (ignored by default for non-bordered child windows, because more convenient)");
