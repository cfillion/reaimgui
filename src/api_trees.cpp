...
"TreeNode functions return true when the node is open, in which case you need to also call TreePop() when you are finished displaying the tree node contents.",
{
  ...
  return ImGui::TreeNodeEx(label, VALUE_OR(flagsInOptional, 0));
});
...
{
  USE_WINDOW(window, false);
  return ImGui::TreeNodeEx(str_id, VALUE_OR(flagsInOptional, 0), "%s", label);
});
...
{
  USE_WINDOW(window, false);
  return ImGui::CollapsingHeader(label, visibleInOptional, VALUE_OR(flagsInOptional, 0));
});
