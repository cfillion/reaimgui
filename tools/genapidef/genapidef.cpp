...
{
  constexpr auto kind { clang::StringLiteral::Ascii };
  clang::StringLiteral::Create(m_ctx, str, kind, false, {}, {})
    ->outputString(out);
};

static std::string_view stripPathAndExt(const std::string_view &fn)
{
  ...
  size_t ext { fn.rfind('.') }, extLen {};
  if(ext != std::string_view::npos)
    extLen = fn.size() - ext;

  return fn.substr(start, fn.size() - start - extLen);
}
...
std::string Function::makeReaperDef() const
{
  ...
  lp << helpText();
  lp << stripPathAndExt(startLoc.getFilename());
  lp << startLoc.getLine();
  lp << endLoc.getLine();
  return out.str();
}
...
class Context; using ImGui_Context = Context;
class Font; using ImGui_Font = Font;
class ImGui_DrawList;
class ImGui_ListClipper;
class ImGui_TextFilter;
class reaper_array;
class Viewport; using ImGui_Viewport = Viewport;
)";
...
int main(int argc, const char *argv[])
{
  ...
  std::string error;
  std::unique_ptr<FixedCompilationDatabase> compilationDatabase
    { FixedCompilationDatabase::loadFromCommandLine(argc, argv, error) };
  ...
  ClangTool tool { *compilationDatabase, { argv[1] } };
  return tool.run(newFrontendActionFactory<GenApiDefAction>().get());
}
